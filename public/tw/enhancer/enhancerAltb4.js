...
var scriptURL = "https://wifm.site/tw/enhancer/";
...
var pausein = parseInt((18 + Math.random() * 4));
var pausedelay = parseInt(((deli * 2) + Math.random() * deli) * 1000);
var timing;
var pauser;
var pause1 = 0
var cid, wood, stone, iron, resstorage;
var tryab = 0
    ...
    setRunning = (running) => {
        if (running) {
            ...
            cid = setInterval(() => {
                ...
                var aButton = window.top.$("#plunder_list tr").filter(":visible").eq(1).children("td").eq(9).children("a");
                var bButton = window.top.$("#plunder_list tr").filter(":visible").eq(1).children("td").eq(10).children("a");
                var cButton = window.top.$("#plunder_list tr").filter(":visible").eq(1).children("td").eq(11).children("a");
                //              var wall=cell.find('span').html()) == "?"
                //              window.top.$("#plunder_list tr").filter(":visible").eq(1).children("td").eq(7).children("a");
                //alert(wall[1]);
                //              var full=window.top.$("#plunder_list tr").filter(":visible").eq(1).children;
                //             alert(full[0]);



                //            if (!tryClick(bButton)) {pause1=0};
                if (pause1 > pausein) {
                    pause1 = 0;
                    setRunning(false);
                    timing = setInterval(() => {
                        pausedelay = pausedelay - 1000;
                        window.top.UI.InfoMessage('Pausing farming...' + parseInt(pausedelay / 1000), 1000);
                        if (pausedelay < 0) {
                            pausedelay = parseInt(((deli * 2) + Math.random() * deli) * 1000);
                            setRunning(true);
                ...
                //                    new Notify('Resuming farming...').show();




                //              if (!document.hidden){selectMasterButton(row);};
                if (!document.hidden && localStorage.now == "대기") {
                    tryClick(bButton);
                    //              var p1 = window.top.$.jStorage.get("profile:" + keyPressSettings.priorityOneProfile);                if (keyPressSettings.priorityOneEnabled && !checkRowToHide(row, p1)) {               tryClick(cButton);}
                    //                if(tryab>1)getNewVillage("n");
                    //                if(tryab==0)tryClick(bButton);
                    //                if(tryab==1)tryClick(aButton);
                }
                if (localStorage.now != "대기") {
                    window.top.UI.InfoMessage('대기상태가 아닙니다..', 7000);
                    //              setTimeout(function(){localStorage.setItem("ball",1);},(Math.random()*5)*1000+41000);
                };

                //if (aButton[0]) {alert("a");tryClick(aButton);};
                //if (bButton[0]) {alert("b");tryClick(bButton);}




                if ($('#am_widget_Farm').length === 0) {
                    setInterval(function() {
                        botc++;
                        bot2();
                    }, 180000);
                    new Notify('Bot protection :/').show();
                    setRunning(false);
                }
...
function count() {
    var pausing = setInterval(() => {
        var pausedelay = pausedelay - 1000;
        window.top.UI.InfoMessage('Pausing farming...' + pausedelay / 6000, 1000);
    }, 1000);
}
...
function incoming() {
    ...
    if (ia == null || ia == undefined) {
        ...
        if (incoming > ia) {
            //	var winRef;	if(typeof(winRef) == 'undefined' || winRef.closed) {var winref = window.open('', 'incoming', '', true);if(winref.location.href === 'about:blank'){	var  newloc=document.URL.split('screen')[0]+"screen=overview_villages&mode=incomings&subtype=attacks&group=0";winref.location.href = newloc;};

            setTimeout(function() {
                window.location.href = document.URL.split('screen')[0] + "screen=overview_villages&mode=incomings&subtype=attacks&group=0";
            }, 1200);
            //var audio = new Audio('https://logboss.net/tw/99/beep-05.wav');audio.loop = true;audio.play();setTimeout(function(){audio.pause();},2000);
        }
    };
}
...
function getNewVillage(way) {
    ...
    wood = parseInt(document.getElementById('wood').innerText);
    stone = parseInt(document.getElementById('stone').innerText);
    iron = parseInt(document.getElementById('iron').innerText);
    resstorage = parseInt(document.getElementById('storage').innerText);
    //    if(wood>resset||stone>resset||iron>resset){alert("full resources"); window.location.href=link[0]+window.top.game_data.village.id +"&screen=snob";}


    //    alert(wood+" "+stone+" "+iron);
    pause1++;
    tryab = 0;
    /*    if (pause1>pausein)
                     {pause1=0;      setRunning(false);
                         timing=setInterval(()=>{pausedelay=pausedelay-1000; window.top.UI.InfoMessage('Pausing farming...'+parseInt(pausedelay/1000),1000);
                         if(pausedelay<0)  {pausedelay=  parseInt((10 + Math.random() * 6) * 60 * 1000);setRunning(true);clearInterval(timing);getNewVillage(way)}},1000);
                         };
    */
    if (way == "n")
        window.top.UI.InfoMessage('Switching to next village...' + pause1 + '/' + pausein, 500);
    else
...
function addLanguages() {
    ...
    window.top.$('#language').append("<option value='el'>횄탐창��▣꺟시궰뼙꺟시궰뼙꺟시궰련꺟시궰시꺟시궰뮨꺟시궰봤꺟시궰�</option>");
    window.top.$('#language').append("<option value='it'>Italiano</option>");
    window.top.$('#language').append("<option value='es'>Espa횄�횂짹ol</option>");
    window.top.$('#language').append("<option value='ar'>횄�횂짠횄�▣™궗탑횄�▣™궗탑횄�횂쨘횄�횂짤 횄�횂짠횄�▣™궗탑횄�횂쨔횄�횂짹횄�횂짢횄�▣� 횄�횂짤</option>");
}
